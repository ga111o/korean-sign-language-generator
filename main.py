...
def main():
    ...
    if not os.path.exists(csv_path):
        logging.error(f"오류: {csv_path} 파일이 존재하지 않습니다.")
        return
    ...
    # dataset 폴더가 존재하는지 확인
    if not os.path.exists(dataset_path):
        logging.error(f"오류: {dataset_path} 폴더가 존재하지 않습니다.")
        return
    ...
    if len(processable_files) == 0:
        logging.error("처리할 수 있는 정면 비디오가 없습니다.")
        return
    ...
    for i, file_info in enumerate(processable_files, 1):
        ...
        logging.info(f"\n====================== 비디오 {i}/{len(processable_files)} 처리 중 ======================")
